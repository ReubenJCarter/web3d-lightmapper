import LightMapperWASM from './wasmdist/lightmapperWasm.mjs';

import { Color3, VertexBuffer, Scene, FreeCamera, Camera, MeshBuilder, Vector3, StandardMaterial, Tools, Mesh, VertexData, Texture, Material, PBRMaterial } from "@babylonjs/core";


//Attaches to a scene, creates a new scene with same engine, and uses to render textures, then reduces to 1 px so a color can be use in the light mapper
...
class LightMapper {
	...
	async run(scene, meshes, mapWidth, mapHeight, sampleCount, bounceCount, lightData,
		...
		for(let mesh of meshes){
			...
			let col = mesh.material && mesh.material.diffuseColor ? mesh.material.diffuseColor : 
                mesh.material && mesh.material.albedoColor ? mesh.material.albedoColor :
                Color3.White();
            ...
            let emissiveColor = mesh?.material?.emissiveColor ? mesh.material.emissiveColor : Color3.Black();
            let emissiveIntensity = mesh?.material?.emissiveIntensity ?? 0; 
		...
		addFunctionToChain(()=>{
			...
			this.wasmLightMapper.Start(vecP, vecN, vecUV, vecLMUV, vecI, vecM, vecT, vecMaterial, this.canvas.width, this.canvas.height, frameCount, bounceCount);
...
//Creates a default standard material for bakes meshes 
function createDefaultMeshMaterial(lmData, modelIndex, bakedData, meshIndex, scene, mesh){
    let mat = bakedData.materials[meshIndex*2+0]; 
    let material = new PBRMaterial('', scene); 
    material.useLightmapAsShadowmap = true; 
    material.lightmapTexture = new Texture(lmData.lightMap, scene, true); 
    material.albedoColor = Color3.FromHexString(mat); 
    material.metallic = 0;
    material.roughness = 1;
    mesh.material = material;
}

//Converts an lmData object back to a set of babylon meshes and adds to scene
function addBakedMeshesToScene(lmData, scene, createMeshMaterial=createDefaultMeshMaterial){
    let bakedMeshes = []; 
    for(let i = 0; i < lmData.sceneData.length; i++){
        ...
        for(let m = 0; m < bakedData.meshes.length / 4; m++){
            ...
            createMeshMaterial(lmData, i, bakedData, m, scene, bakedSceneMesh);
            
            bakedMeshes.push(bakedSceneMesh);
        }
    }
    return bakedMeshes; 
}

export {LightMapper, createDefaultMeshMaterial, addBakedMeshesToScene, } 